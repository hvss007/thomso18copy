    ...
    deletePost = () => {
        if (!this.state.deleteDisabled) {
            if (this.props.data && this.props.data._id && this.state.deleted !== undefined) {
                ...
                FetchApi('DELETE',`/api/ca/admin/temp/idea/${this.props.data._id}`, {deleted}, authtoken)
                    .then((result) => {
    ...
    submitComment = () => {
        if (!this.state.submitDisabed) {
            if (this.state.comment) {
                ...
                if (this.props.data && this.props.data._id && comment !== undefined) {
                    this.setState({submitDisabed: true})
                    const authtoken = this.Auth.getToken()
                    FetchApi('PUT',`/api/ca/admin/temp/idea/${this.props.data._id}`, {comment}, authtoken)
                        .then((result) => {
                            if (result && result.data && result.data.body && result.data.body.comment) {
                                ...
                                this.setState({submitDisabed: false, editing: false, message: 'Comment Removed', isVisible: true})
                                timeout = setTimeout(() => this.setState({ isVisible: false }), 3000)
                            }
                        })
                        .catch(error => {
